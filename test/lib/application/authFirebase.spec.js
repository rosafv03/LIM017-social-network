...
describe('testeando la función registrar', () => {
  it('funciona y recibe los argumentos correctos', async () => {
    ...
    expect(JSON.stringify(resolved)).toBe(JSON.stringify({ user: { email: 'miemail@gmail.com', password: '123456' } }));
...
describe('testeando la función login', () => {
  it('funciona y recibe los argumentos correctos', async () => {
    ...
    expect(resolvedLogin).toStrictEqual({ user: { email: 'miemail@gmail.com', password: '123456' } });
  });
});
